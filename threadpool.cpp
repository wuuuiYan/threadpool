...
ThreadPool::~ThreadPool()
{
	...
	// 等待线程池中的所有线程返回，此时线程有两种状态：等待获取任务处于阻塞状态 & 正在执行任务中
...
Result ThreadPool::submitTask(std::shared_ptr<Task> sp)
{
		...
		&& curThreadSize_ < threadSizeThresHold_)
	{
		...
		// 创建新线程对象，生成一个可调用线程函数`threadFunc()`的匿名可调用对象，传递给`Thread`类的构造函数
...
void ThreadPool::threadFunc(int threadId) // 线程函数返回，相应的线程也就结束了！！！
{
	...
	// 所有任务必须执行完成，线程池才可以回收所有线程资源，而不是线程池析构时无论任务有没有执行完成就立刻回收所有线程资源，所以还是用for循环
	// 当任务队列为空时才判断线程池是否析构。最外层的循环必须是死循环，这样才能保证任务全部被执行完再被回收资源
	for (; ; ) // 不断消费任务，所以是一个死循环
	{
...
void Task::exec()
{
	if (result_ != nullptr)
	{
		...
		// 将派生类重写的成员函数run()的返回值Any类型的变量作为实参，调用Result类型的成员函数setVal
	}
	// 模板方法模式：执行框架不改变，具体执行内容采用虚函数绑定
}
